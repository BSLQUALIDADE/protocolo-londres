...
function criarNovoProtocolo() {
    ...
    meta[uuid] = {
        uuid: uuid,
        numero: null, // gerado depois que etapa 1+2 estiverem completas
        status: 'rascunho', // rascunho -> aberto -> em_analise -> encerrado
        nome_residente: null,
...
function listarProtocolosComNumero() {
    return listarProtocolos().filter(p => p.numero);
}
...
class FileUploadManager {
    ...
    renderList() {
        ...
        this.files.forEach((file, idx) => {
            const icon = file.type && file.type.includes('pdf') ? '📄' : (file.type && file.type.includes('image') ? '🖼️' : '📎');
            html += `<div class="file-item"><span class="file-name">${icon} ${file.name} <span class="file-size">${this.formatSize(file.size)}</span></span><button type="button" class="btn-icon delete" data-idx="${idx}">🗑️</button></div>`;
        });
